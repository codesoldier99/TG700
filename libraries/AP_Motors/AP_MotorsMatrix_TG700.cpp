/*
 * ============================================================================
 * TG700 PATCH: AP_MotorsMatrix_TG700.cpp  (v8.0)
 *
 ...
 * 日期: 2026-08-31
 * 版本: v8.0 (新增 ±Y 单向倾角方案并设为默认; 三方案由 Q_FRAME_TYPE 选择)
...
/*
 * ============================================================================
 * 三种混控方案 (由 Q_FRAME_TYPE 选择)
 * ============================================================================
 *
 *   Q_FRAME_TYPE   机架串         倾角要求                       备注
 *   0 或 1         COAX16_AY     只需 ±Y (左/右)                【默认, 当前机械可实现】
 *   2              COAX16_AX4    需要 ±X 与 ±Y 两种             底座若能前后倾才可用
 *   20 或 21       COAX16_COR    需要 ±X 与 ±Y, 且反转 8 台电机  同转棋盘, 权限无增益
 *
 *   偏航因子的物理来源对三者相同:
 *       每台电机的偏航贡献 = sin(倾角) × 力矩臂 + C_Q/C_T × R_prop
 *   水平力沿 ±X 时力矩臂 = |Y|;  沿 ±Y 时力矩臂 = |X|。
 *
 * ---------------------------------------------------------------------------
 * 方案 Y — ±Y 单向倾角 (Q_FRAME_TYPE = 0 或 1, "COAX16_AY")  【默认, v8.0】
 * ---------------------------------------------------------------------------
 *   适用于底座**只能左右倾、无法前后倾**的现实约束 (2026-08-31 现场确认)。
 *
 *   转向: 每组上层 CW / 下层 CCW —— 与现役硬件一致, 桨与 ESC **不需要任何改动**。
 *
 *   力矩臂: 水平力全部沿 ±Y, 故 Mz = x·Fy, 力矩臂 = |X| = 1.665 m。
 *           16 台电机的力矩臂**全部相同** —— 外侧组失去了原先 3.400 m 的长臂,
 *           这是本约束下无法回避的损失, 不是调参能补的。
 *
 *   倾角安装规则 (整个方案只有这一句):
 *
 *           前排 4 组 (P1-P4): 上层朝**左**(-Y), 下层朝**右**(+Y)
 *           后排 4 组 (P5-P8): 上层朝**右**(+Y), 下层朝**左**(-Y)
 *
 *           内外环不再区分, 同一排的 4 组装法完全一样。
 *           推导: 上层 CW 需 Mz<0。前排 x>0 -> 需 Fy<0 -> 朝左;
 *                 后排 x<0 -> 需 Fy>0 -> 朝右。下层 CCW 取反。
 *
 *   偏航因子: 16 台幅值**全部相等** = (sin5°×1.665 + 0.0056)/3.4 = 0.0443,
 *             符号仍是上层负/下层正。经 normalise_rpy_factors() 后全部变成
 *             ±0.5, 即打偏航舵时 16 台电机等量参与 —— 这是本约束下的最优分配。
 *
 *   解耦: 偏航仍在每个共轴组**内部**产生 (上 +Δ, 下 -Δ), 组内推力和恒定,
 *         故对 roll/pitch/升力的扰动**精确为零** (校核脚本实测 0.000e+00)。
 *
 *   偏航权限: 为方案 AX4(5°) 的 79.9%, 为理想切向(5°)的 65.1%。
 *             把倾角加到 6.31° 即可追平 AX4 的 5°, 升力代价仅 0.61% (700kg 折 4.2kg);
 *             加到 8° 为 AX4 的 1.26 倍 (代价 0.97%, 6.8kg);
 *             加到 10° 为 1.56 倍 (代价 1.52%, 10.6kg)。**强烈建议加大到 8°-10°**。
 *
 *   与机上 v4.0 固件的关系: v4.0 的外/内因子比是 1.640 (切向假设的产物),
 *             而 ±Y 约束下真实比值是 1.000。装成本方案后 v4.0 **方向正确、解耦正确、
 *             可以安全飞**, 但只能发挥这套底座 80.5% 的偏航权限 —— 它把外侧电机
 *             驱动得过重, 外侧先饱和而内侧余量闲置。本方案修正了这个失配。
 *
 * ---------------------------------------------------------------------------
 * 方案 A — 对转, ±X/±Y 四向 (Q_FRAME_TYPE = 2, "COAX16_AX4")  【v6.0, 需 X 向倾角】
 * ---------------------------------------------------------------------------
 *   四向里取力矩臂最大的:
 *     外侧: |Y| = 3.400 > |X| = 1.665  -> 外侧 4 组沿 ±X (机头/机尾)
 *     内侧: |Y| = 1.600 < |X| = 1.665  -> 内侧 4 组沿 ±Y (左/右)
 *   偏航权限最高, 但要求底座能前后倾。若机械条件恢复, 优先用本方案。
 *
 ...
 *
 * ---------------------------------------------------------------------------
 * 方案 B — 同转棋盘, ±X/±Y 四向 (Q_FRAME_TYPE = 20/21, "COAX16_COR") 【v5.0】
 * ---------------------------------------------------------------------------
 *   转向: 每组上下两台**同向**, 组间棋盘交替
 *           CW  组: P1, P3, P6, P8
 *           CCW 组: P2, P4, P5, P7
...
// ±Y 单向倾角 (方案 Y): 力矩臂对 16 台电机一律是 |X| = 1.665 m, 故幅值全部相同。
//   (0.08716 × 1.665 + 0.0056) / 3.4 = 0.0443
// 外侧组在此约束下并不比内侧强, 因此不能再沿用外/内 = 1.640 的加权。
#define TG700_YAW_ALL               ( 0.0443f)
#define TG700_YA_CW                 (-TG700_YAW_ALL)
#define TG700_YA_CCW                ( TG700_YAW_ALL)
...
bool AP_MotorsMatrix::setup_motors_tg700(motor_frame_type frame_type)
{
    ...
    // 方案 Y: ±Y 单向倾角 —— 转向同方案 A (上 CW / 下 CCW), 但 16 台幅值相等。
    //   倾角: 前排(P1-P4) 上朝左/下朝右;  后排(P5-P8) 上朝右/下朝左。
    static const float yaw_y_only[16] = {
        TG700_YA_CW,  TG700_YA_CCW,   // P1 上/下  前排
        TG700_YA_CW,  TG700_YA_CCW,   // P2 上/下  前排
        TG700_YA_CW,  TG700_YA_CCW,   // P3 上/下  前排
        TG700_YA_CW,  TG700_YA_CCW,   // P4 上/下  前排
        TG700_YA_CW,  TG700_YA_CCW,   // P5 上/下  后排
        TG700_YA_CW,  TG700_YA_CCW,   // P6 上/下  后排
        TG700_YA_CW,  TG700_YA_CCW,   // P7 上/下  后排
        TG700_YA_CW,  TG700_YA_CCW,   // P8 上/下  后排
    };
    ...
    switch (frame_type) {
    case MOTOR_FRAME_TYPE_PLUS:         // 0 — 机上现有参数值, 默认走 ±Y 单向方案
    case MOTOR_FRAME_TYPE_X:            // 1
        _frame_type_string = "COAX16_AY";
        yaw = yaw_y_only;
        break;
    case MOTOR_FRAME_TYPE_V:            // 2 — 需底座能沿 ±X 前后倾
